...
client.on('connectFailed', function(error) {
    console.log('[WebSocket service] WebSocket Connect Failed: ' + error.toString());
});
...
client.on('connect', function(connection) {
  ...
  connection.on('error', function(error) {
      console.log("[WebSocket service] Connection Error: " + error.toString());
  });
  ...
  connection.on('message', function(message) {
    if (message.type === 'utf8') {
      ...
      PostOne('RSM', obj, 0, function(resCode, resMsg, times) {
        if (resCode !== 200) {
          console.debug("[WebSocket service] Save file to RSM collection failed!");
          return;
        } 
