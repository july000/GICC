...
function scheduleDeleteRSM() {
	var jobInstance = schedule.scheduleJob("*/5 * * * *", () => {
		...
		var now = new Date();
		var deleteStartTimeDate = new Date(
			now.getFullYear(),
			now.getMonth(),
			now.getDate(),
			now.getHours(),
		...
		var trafficEventStartTimeDate = new Date(
			now.getTime() + timezoneOffset * 60 * 1000 - 5 * 60 * 1000
		);
		var traficEventEndTimeDate = new Date(
		...
		getTrafficEvent(traficEventStartTime, traficEventEndTime)
			.then((data) => {
				...
				DeleteAll("RSM", {
					"data.timestamp": {
						$lte: Date.parse(deleteStartTimeDateString),
					},
				});
			})
			.catch((error) => {
...
function filterDataToRSMEvent(trafficEventList) {
	if (trafficEventList.length === 0) {
		console.log(
			"[schedule service] trafficEventList is empty. Processing..."
		);
		...
		promises.push(
			new Promise((resolve, reject) => {
				...
				Get("RSM", query, i, function (resCode, resMsg, times, Obj) {
					if (resCode !== 200) {
						console.error(
							"[schedule service] GetOne event RSM file failed ---------------" +
								resCode +
								"-----------------"
						);
						reject(resCode);
					...
					if (Obj) {
						...
						Obj.forEach((element) => {
							...
							PutOne(
								"RSM_Event",
								{ "data.timestamp": element.data.timestamp },
								element,
								0,
								function (resCode, resMsg, times) {
									if (resCode !== 200) {
										console.debug(
											"[schedule service] post to RSM_Event failed!"
										);
										return;
									}
...
function scheduleDeleteRSMEvent() {
	var jobInstance = schedule.scheduleJob("0 0 0 1 * *", () => {
		...
		DeleteAll("RSM_Event", {
			"data.timestamp": { $lte: Date.parse(startTime) },
		});
	});
}
