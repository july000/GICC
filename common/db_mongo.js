...
function MongoInit(projectName, proInfo, callback) { // var currProName = require("./IniConfig").fesConfig("projectName");
...
function MongoFindOne_2D(collName, query, times, t1, t2, callback) {
    console.log("callback in MongoFindOne: times=" + times + " t1=" + t1 + " t2=" + t2);
    var Schema = SchemaSelector(collName);
    var DataModel = SetSchema(Schema, collName);
...
function MongoUpdate(collName, query, new_item, times, callback) {     // query must be uniquely identified
    var Schema = SchemaSelector(collName);
    var DataModel = SetSchema(Schema, collName);
...
function MongoDrop(dbName, callback) {
    if (dbName === global.projectName) {
        ...
        // });
        mongoose.connect('mongodb://10.0.8.239:27017/' + dbName, {useMongoClient: true})
		.then(result => {
            	mongoose.connection.db.dropDatabase();
            	console.debug("DB " + dbName + " dropped!");
            	callback(200, "DB " + dbName + " dropped!");
...
function MongoGetDBs(callback){
    ...
    var connection = mongoose.createConnection('mongodb://127.0.0.1:27017/' + global.projectName);
            ...
            if(err){
                ...
                for(var i = 0; i < result.databases.length; i++){
                    ...
                    if(result.databases[i].name === global.projectName){
                        MongoFindOne('node', {"id": 1}, i, function(rspCode, rspMsg, times, obj){
                            var curPro = {id: 1, name: result.databases[times].name, description: '', default: true};
                            curPro.project = obj;
                            pros.push(curPro);
                        ...
                        var pro = {id: 1, name: result.databases[i].name, description: '', default: false};
                        pros.push(pro);
                        if(pros.length === result.databases.length){
                            callback(200, "MongoGetDBs success!", pros);
                        }
