	...
	try {
		import("geodesy/latlon-ellipsoidal-vincenty.js")
			.then((module) => {
				...
				const north =
					-sinLat1 * cosLon1 * deltaLat -
					sinLat1 * sinLon1 * deltaLon +
					cosLat1 * deltaHeight;
				const up =
					cosLat1 * cosLon1 * deltaLat +
					cosLat1 * sinLon1 * deltaLon +
					sinLat1 * deltaHeight;
...
async function writeParticipantDataToCsv(
	participantData,
	participantFields,
	outputPath
) {
	const updatedData = await Promise.all(
		participantData.map(async (data) => {
...
function rsm_to_dataverse(documents, outputFile) {
	for (const document of documents) {
		...
		var extractedData = extractParticipantData(
			transformedData,
			participantCount,
			csvFields
		);
